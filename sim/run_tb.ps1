...
iverilog -o (Join-Path $Work "tb_pipe.out") `
    (Join-Path $Root "rtl\controller\hardwired_ctrl_core.v") `
    (Join-Path $Root "rtl\controller\pipe_regs.v") `
    (Join-Path $Root "rtl\controller\hazard_unit.v") `
...
iverilog -o (Join-Path $Work "tb_pipe_board.out") `
    (Join-Path $Root "rtl\controller\hardwired_ctrl_core.v") `
    (Join-Path $Root "rtl\controller\pipe_regs.v") `
    (Join-Path $Root "rtl\controller\hazard_unit.v") `
    (Join-Path $Root "rtl\controller\hardwired_ctrl_pipe.v") `
    (Join-Path $Root "sim\tb_pipe_board.v")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

vvp (Join-Path $Work "tb_pipe_board.out")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
