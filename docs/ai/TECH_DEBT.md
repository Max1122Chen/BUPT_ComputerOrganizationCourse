...
| TD-004 | OUT/DI/EI/IRET 控制字（拓展/自设计） | CTL | **Low** | **Mitigated** | CTL-F02 | 已实现于 `hardwired_ctrl.v`；板上待测 |
| TD-005 | STO 信号未接入控制器 | CTL | **Low** | **Done** | `rtl/common/manual_sto.v` | FPGA 内 W1 末拍翻转 STO |
| TD-006 | 中断拓展专题 | CTL | **Low** | **Mitigated** | CTL-F02 | INTR/LIAR/IABUS + W1 序列；板上待测 |
